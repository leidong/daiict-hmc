...
if ( ! class_exists('XmlExportEngine') ){
	...
	{		
		...
		public function get_available_data(){			
			...
			if ( self::$is_user_export )
			{
				$this->available_data['init_fields'] = $this->user_export->get('init_fields');
				$this->available_data['default_fields'] = $this->user_export->get('default_fields');
				$this->available_data['advanced_fields'] = $this->user_export->get('advanced_fields');
			}
			else
			{
				if ( XmlExportWooCommerceOrder::$is_active )
				{
					$this->available_data['init_fields'] = $this->woo_order_export->get('init_fields');
				}
				else
				{
					$this->available_data['init_fields'] = $this->init_fields;
					$this->available_data['default_fields'] = $this->default_fields;
					$this->available_data['other_fields'] = $this->other_fields;

					$this->available_data['woo_data'] = $this->woo_export->get('_woo_data');
					$this->available_data['existing_attributes'] = $this->woo_export->get('_existing_attributes');
		...
		public function render(){
			...
			if ( ! XmlExportWooCommerceOrder::$is_active )
			{
				...
				// Render Available WooCommerce Data
				$this->woo_export->render($i);
			}
		...
		public function render_filters(){
			...
			$filter_sections = $this->filter_sections;

			if (self::$is_user_export) $filter_sections = $this->user_export->get('filter_sections');
			if (XmlExportWooCommerceOrder::$is_active) $filter_sections = $this->woo_order_export->get('filter_sections');			
			...
			foreach ($available_sections as $slug => $section) {							
				...
				if ( in_array($section['content'], array('default_fields', 'other_fields'))) continue;
				...
				if ( ! empty($this->available_data[$section['content']]) ){
					...
					<optgroup label="<?php echo $section['title']; ?>">								
						<?php
						if (self::$is_user_export){
							foreach ($this->available_data['other_fields'] as $field) {	
								?>
			...
			if ( ! XmlExportWooCommerceOrder::$is_active )
			{
				...
				// Render Available WooCommerce Data
				$this->woo_export->render_filters();			
			}
