				...
				switch ($rule->element) {
					...
					default:
						...
						if (strpos($rule->element, "cf_") === 0)
						{
							...
							{
								$this->queryJoin[] = " INNER JOIN $wpdb->postmeta ON ($wpdb->posts.ID = $wpdb->postmeta.post_id) ";
								$this->queryWhere .= "$wpdb->postmeta.meta_key = '$meta_key' AND $wpdb->postmeta.meta_value " . $this->parse_condition($rule);
							}										
